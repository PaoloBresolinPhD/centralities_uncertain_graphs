...
void run_single_experiment(UncertainGraph &uncertain_graph, std::string &output_dir_path, int k_baseline, double c_ew, int k, int l, std::mt19937 &rng) {
    ...
    auto start_exact_baseline = std::chrono::high_resolution_clock::now();
    ...
    auto end_exact_baseline = std::chrono::high_resolution_clock::now();

    // append the centralities
    table.push_back(exact_centralities_baseline);

    // compute and append the time required to compute the centralities
    std::chrono::duration<double> elapsed_exact_baseline = end_exact_baseline - start_exact_baseline;
    exec_times.push_back(elapsed_exact_baseline.count());
    ...
    exec_times.push_back(elapsed_exact.count());
    ...
    // run the ew algorithm on the loaded uncertain graph
    std::cout << "Running ew with k = " << k << ", l = " << l << " and c = " << c_ew << std::endl;
    auto start_ew = std::chrono::high_resolution_clock::now();
    std::vector<double> ew_centralities = mc_centralities_uncertain_graph(uncertain_graph, k, ew_lin_world_adapter, rng);
    auto end_ew = std::chrono::high_resolution_clock::now();

    // append the centralities
    table.push_back(ew_centralities);

    // compute and append the time required to compute the centralities
    std::chrono::duration<double> elapsed_ew = end_ew - start_ew;
    exec_times.push_back(elapsed_ew.count());
    ...
    std::filesystem::path centralities_path = output_dir / "centralities.tsv";
    save_centralities_tsv(centralities_path, table, method_names);
